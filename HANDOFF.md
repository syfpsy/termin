...
## Polish + Distribution Pass (2026-04-25)

- **Landing page**: `#start` surface beefed into a real first-visit page with 6-card promise grid, 6 library seed cards (engine-rendered MiniPreview), and a footer linking to GitHub. Sets context before dropping users into the studio.
- **Docs surface**: `#docs` view with sidebar TOC + 6 sections (.me syntax, bundle schema, web player, keyframes, audio, keyboard). All inline code blocks use the JetBrains Mono terminal type.
- **Embed code generator**: new panel on the export surface with six output formats (Web Component, iframe, shareable URL, React, Vue 3, Svelte). User-editable origin field, copy-to-clipboard with confirmation flash, regenerate-loop-URL button for iframe / share modes.
- **Per-event property keyframes** (was deferred, shipped 2026-04-25): `prop event-N intensity 0ms 0.5 200ms 1.0` syntax targets a specific event by source line. Parser validates against the `AnimatableEventParam` set (today: `intensity`); engine pre-computes a per-event scale before each `applyPrimitive` call and every `grid.set` / `grid.writeText` site multiplies its intensity through the new `scaled(value, ctx)` helper. Bundle-compiled animations now carry `eventLine: number | null`. Smoke covers parse, sample-at-endpoints, sample-with-easing, format round-trip, unknown event/param fallback to identity, bundle preserves eventLine.

## Still Deferred (need engine design pass)

- **Color / font / mode keyframes**: extending the keyframe model from numeric to discrete (mode, font) and color (tone). Color animation is high-value but requires a tone-interpolation strategy (RGB lerp, HSL lerp, or named-tone hold) and per-cell color tracking — today every cell stores a single `ToneName`, not an RGB triple. Font / mode are step-style and can land trivially with `hold` easing; the renderer just needs to read the sampled discrete value per tick.
- **Server-side patch protocol for the director**: today the director returns a full DSL replacement and the diff view shows the change. A cleaner model is for the director to emit structured patch ops (`{op: 'move', eventLine: 3, atMs: 1500}`, `{op: 'add', effect: 'glitch', atMs: 2000}`) so the AI can edit a long scene without rewriting it. Wire a `mode: 'patch' | 'replace'` field on `DirectorProposal` and apply patches via the existing `dslEdit` helpers when present.
- **Open Graph image generation**: a `/api/og` endpoint that takes a `play=` fragment and emits a PNG poster of the scene's peak frame. Vercel's `@vercel/og` is the obvious dep; saves an SVG-to-PNG render so a Slack / Twitter paste of a loop URL unfurls with a real preview. ~half session.
- **Community gallery / hosted scenes**: server-stored loop bundles in Vercel Blob with vote / fork / publish flow. Closes the distribution loop. Real scope, real auth (or open / public-only).
