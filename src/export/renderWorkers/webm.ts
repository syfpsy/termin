...
export async function renderWebmClip(input: WebmInput): Promise<Blob> {
  ...
  if (!mimeType) throw new Error('No supported WebM codec found for MediaRecorder.');
  ...
  const stopped = new Promise<void>((resolve, reject) => {
    ...
    recorder.addEventListener('error', (event) => reject((event as ErrorEvent).error ?? new Error('MediaRecorder failed.')));
